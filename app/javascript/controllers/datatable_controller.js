...
export default class extends Controller {
    ...
    static values = {
        order: {           // Default order (parsed from JSON)
            type: Array, default: []    // Default to empty array if not provided
        }
    }

    connect() {
        // 1. Check if the table is already initialized via the DataTables API
        if (DataTable.isDataTable(this.element)) {
            console.warn("DataTable already initialized. Skipping reinitialization.");
            // If it's already initialized, just return
            return;
        }

        // 2. Fallback check (your current method) - good to keep as a second line of defense
        if (this.element.dataset.dataTableInitialized) {
            console.warn("DataTable already initialized via dataset. Skipping reinitialization.");
            return;
        }
    ...
    initializeDataTable() {
        this.element.classList.add('table', 'stripe', 'hover');
            ...
            buttons: [
                {
                    ...
                    buttons: [{
                        extend: 'copyHtml5',
                        text: '<span class="flex w-full items-center gap-x-2 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100 dark:text-neutral-400 dark:hover:bg-neutral-700 dark:hover:text-neutral-300 dark:focus:bg-neutral-700">' + '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-copy-icon lucide-copy shrink-0 size-4"><rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/></svg> Copy</span>',
                        titleAttr: 'Copy',
                        exportOptions: {
                ...
                {
                    extend: 'colvis', columns: ':not(.no-export)'
                }],
            language: {
                ...
                const dropdownInputs = document.querySelectorAll('select.dt-input');
                if (dropdownInputs.length > 0) { // Check if any elements were found
                    dropdownInputs.forEach(input => {
                        input.style.paddingLeft = '0.5rem'; // 8px
                    });
                }

                const searchInputs = document.querySelectorAll('input.dt-input'); // Assuming tableContainer is defined from the DataTables instance
                searchInputs.forEach(searchInput => {
                    // Apply styles directly to the search input
                    ...
                    // Create a wrapper div for each search input
                    const wrapper = document.createElement('div');
                    wrapper.style.position = 'relative';
                    wrapper.classList.add('relative'); // Add relative class for tailwind.

                    // Get the parent of the current searchInput
                    const searchInputParent = searchInput.parentNode;

                    // Check if the searchInputParent exists and if the searchInput is actually a child of it
                    if (searchInputParent && searchInputParent.contains(searchInput)) {
                        // Replace the searchInput with the wrapper
                        searchInputParent.replaceChild(wrapper, searchInput);
                        // Move the searchInput inside the new wrapper
                        wrapper.appendChild(searchInput);
                    } else {
                        // Fallback or error handling if searchInput's parent isn't as expected
                        // This might happen if DataTables structure is different than anticipated
                        console.warn('Could not find parent for search input or search input is already moved.');
                        // If it's already moved, just append the SVG to the current wrapper if it exists
                        if (searchInput.parentNode && searchInput.parentNode.classList.contains('relative')) {
                            searchInput.parentNode.insertAdjacentHTML('beforeend', `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search-icon lucide-search absolute left-2 top-1/2 transform -translate-y-1/2 ps-1 shrink-0 size-5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>`);
                        } else {
                            // If no parent or expected wrapper, just append the SVG next to the input
                            searchInput.insertAdjacentHTML('afterend', `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-search-icon lucide-search absolute left-2 top-1/2 transform -translate-y-1/2 ps-1 shrink-0 size-5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>`);
                            console.warn('SVG might not be positioned correctly due to unexpected DOM structure.');
                        }
                        return; // Skip remaining logic for this specific input if wrapper couldn't be set up
                    }


                    // Insert the SVG inside the newly created wrapper
                ...
                const btnComps = document.querySelectorAll('.dt-buttons');
                if (btnComps) {
                    btnComps.forEach(btnComp => {
                ...
                const exportButtons = document.querySelectorAll('button.dt-button.buttons-collection');
                exportButtons.forEach(exportButton => {
                    // Check if icons have already been added to this specific button
                    if (!exportButton.dataset.iconsAdded) {
                        // Ensure the button is displayed as flex to align content and icons
                        exportButton.style.display = 'flex';
                        exportButton.style.alignItems = 'center'; // Vertically center the content

                        // Add the download icon to the beginning of the button
                        ...
                        // Add the dropdown arrow icon to the end of the button
                        ...
                        // Set a flag on the button to indicate icons have been added
                        exportButton.dataset.iconsAdded = 'true';
                    }
                });


                const colvisButtons = document.querySelectorAll('button.dt-button.buttons-collection.buttons-colvis');
                colvisButtons.forEach(colvisButton => {
                    // Check if the icon has already been added to this specific button
                    if (!colvisButton.dataset.iconsAdded) { // This is the crucial line!
                        // Ensure the button is displayed as flex to align content and icons
                        colvisButton.style.display = 'flex';
                        colvisButton.style.alignItems = 'center'; // Vertically center the content
                        ...
                        // Set a flag on the button to indicate icons have been added
                        colvisButton.dataset.iconsAdded = 'true'; // Set the flag!
                    }
                });
